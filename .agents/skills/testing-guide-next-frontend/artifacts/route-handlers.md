...
For handlers with route params, build a `NextRequest` and pass the `{ params }` context object the handler expects (see `.agents/rules/next-frontend-bff-api.md` for the param signature).
