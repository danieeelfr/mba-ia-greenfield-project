...
**SI-NN.X.0 — Drift audit (precedes every SI-Xa):** load `figma:figma-implement-design` plugin skill. The audit's purpose is to surface DS-component drift to the user via `frontend-drift-report.md` BEFORE SI-Xa applies any DS file edits. The audit RECORDS decisions; it does NOT apply edits. Schema reference: `.agents/skills/plan-build/references/frontend-drift-report-schema.md`.
...
- `Bash test -f .agents/rules/design-system.md` (typical project path; adjust per project convention).
- If present: `Grep -n '^## .* Aliases' .agents/rules/design-system.md` to locate the aliases section → bounded-Read it (upper bound: next `^## ` heading or end of file). Parse the table into an in-memory map: `(figma_value, component_path) → code_variant`. Component scope `*` matches all paths.
...
Follow the target subproject's conventions. When you need to discover naming/style/structure patterns of the subproject (e.g., "how are services organized", "what's the module structure convention"), use `grep_search` and `view_file` to read neighbor files directly. Only read what's needed to understand the convention — don't load full files when only signatures or folder structure is needed.

Scope note: Convention discovery (file naming, folder layout, local helpers, repository idioms) is separate from the loaded best-practices skills (e.g., `nestjs-best-practices`, `typeorm`). If the convention is already documented by a loaded skill, use that — don't read source files redundantly.

**Convention discovery patterns** — use the minimum sufficient read:
- Learning a convention → `grep_search` for patterns + `view_file` with `StartLine`/`EndLine` for a representative snippet (30–40 lines).
- Understanding a neighboring class or API → `grep_search` for the class/method name to locate its line, then `view_file` with targeted range (signatures only, not bodies).
- Discovering folder layout → `list_dir` — no file contents.

Never read a 500-line file when you only need 30 lines of it.

**[Antigravity note]** The Claude Code `Agent` tool with `subagent_type: Explore` does not exist in Antigravity. All convention discovery is done directly with `grep_search`, `view_file`, and `list_dir` in the main thread.
...
**[Antigravity adaptation]** Execute tests directly with `run_command`. Read `{target-subproject}/AGENTS.md` (already loaded in context as AGENTS.md) for the exact command conventions — containerization, env vars, etc.

Run **only** the test files listed in the SI's Tests section — never the full suite. When `PLAN_MODE=modern` AND Step 3a authored spec-derived files, the test list is the **union** of (Set A) the SI's inline Tests-section paths and (Set B) the spec-derived files from working memory.

**nestjs-project pattern** (from AGENTS.md):
```bash
# Run specific unit/integration test:
docker compose exec nestjs-api npm test -- --runInBand path/to/file.spec.ts

# Run specific e2e test:
docker compose exec nestjs-api npm run test:e2e -- --testPathPattern="filename"
```

**Diagnostic contract** — from the raw output:
- **Preserve verbatim:** test names, failure messages, assertion diffs, stack frames into project code, console output, timeouts, unhandled rejections.
- **Summarize:** passing tests as "N tests passed". Strip framework-internal stack frames and startup banners.
- For fully passing runs: note "all N tests pass" and proceed.

**Rule:** if the output is insufficient to diagnose the failure, re-run with verbose flags or check container logs (`docker compose logs nestjs-api`). Never retry blindly without understanding the failure.
...
- Run only the SI's own tests during the loop, via the subagent pattern defined in step 4 — which requires the test subagent to first Read `{subproject}/AGENTS.md` for command conventions (subagents inherit nothing from the Antigravity context). Save the full suite for final verification.
