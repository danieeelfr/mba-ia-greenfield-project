...
> **[Antigravity adaptation]** This skill was originally a Claude Code sub-agent. In Antigravity, its instructions are applied directly by the main agent using native tools: `grep_search` (≡ Grep), `view_file` (≡ Read/bounded Read), `list_dir` (≡ Glob), `run_command` (≡ Bash), `ask_question` (≡ AskUserQuestion).
